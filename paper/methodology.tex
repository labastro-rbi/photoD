...
\redcom{LP: "In summary", instead of "Therefore", or am I missing something? Equations could be moved to the top of the paragraph, this is somewhat repetitive.} Therefore, $\vec{D}$ = ($r$, $\vec{c}$) and $\vec{\theta}$ = ($M_r$, $[Fe/H]$, $A_r$). Data $\vec{D}$ and
...
its integral over all model parameters $\vec{\theta}$ is unity. \redcom{LP: A zašto?} The integral of the prior $p(\vec{\theta}|M,I)$
...
below, where the correct normalization of the product $p(D|M,\vec{\theta},I) \, p(\vec{\theta}|M,I)$ is crucial. \redcom{LP: OK, ovo mi baš nije bilo jasno...}
...
The symbols \redcom{"ribbons"?} are color-coded by metallicity, ranging from $[Fe/H] = -2.5$ to 0 (blue to red). The three lines correspond to three values of
...
using eqs. A2 and A7 from Ivezi\'{c} et al. (2008). \redcom{very difficult to discern...} Note the discrepancy between these parametrizations and data for sub-giant stars ($M_r \sim 3-4$
and $g-i \sim 0.8-1.1$). The right panel shows a sample of 415,000 stars with $r < 22$ and $u<22$ as red dots (shown by blue dots in Figure~\ref{fig:3dataDiags}(\redcom{corrected to Fig 1; was this the intention?}), and 409,000 of those that also have  $0.2 < g-i < 3.5$ as cyan dots. Their absolute magnitudes were computed
...
sample) seen at $g-i = 0.4$ and $Mr > 7$ are predominantly found at the faint end ($r>20$).(\redcom{There is some interesting structure in the right plot...but I don't get exactly što je pjesnik želio reći s ovim plotom?})}
\label{fig:3HRdiags}
\end{figure*}
...
populations to model-based isochrones that span a range of ages and a wider range of metallicities \redcom{where the former fail to properly explain the observations?} 
...
\redcom{Tu sam bio malo zbunjen: u zadnjem odlomku iz 2.2.1. piše: da se prebacimo na model-based izokrone; čitanjem 2.2.2. ispada da su modeli samo nadoštukani? }
...
with extincted colors generated using different values of $C_m$. \redcom{If we are constraining $A_r$ from data that are fitted to the models, what does this loop look like...? For each LoS we fit different models and stop when some convergence criteria is satisfied? Is this better than taking a prior from TRILEGAL, or at the least using TRILEGAL to constrain the possible models? OK, only a small area is possibly affected by this but IIRC,in the central regions there are clumps so the results may look too "smooth".}
...
\begin{equation}
          A_r^{max} =  a \, A_r^{SFD} \, + \, b,
\end{equation}
and $a=1.3$, $b=0.1$. Therefore, $p(A_r) = 1/A_r^{max}$ for $0 \le A_r \le A_r^{max}$ and $p(A_r) = 0$ for $A_r > A_r^{max}$. \redcom{Why those exact numbers?}
