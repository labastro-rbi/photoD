# Added to force loading bt, lt and pt versions from this dir:
import sys
sys.path.append('.')
...
def writeBayesEstimates(catalog, outfile, iStart, iEnd, do3D=False):
    fout = open(outfile, "w")
    if do3D:
        fout.write("      glon       glat        FeHEst FeHUnc  MrEst  MrUnc  ArEst  ArUnc  chi2min    MrdS     FeHdS  ArdS  cnt \n")
    else:
...
def checkBayes(infile1, infile2, chi2max=10, umagMax=99.9, chiTest=False, cmd=False, fitQ=False, b3D=False):
    ...
    if (umagMax < 99):
        ...
        print('from', np.size(simsAll),' selected flux-limited with u <', umagMax, np.size(sims))
    ...
    simsTestChi = sims[simsBayes['chi2min']<chi2max]
    print('after chi2min<', chi2max, 'selection:', np.size(sims), np.size(simsTestChi), 'may include WDs etc')  

    onlyMSRG = True
    if (onlyMSRG):
        simsTest = simsTestChi[(simsTestChi['logg']<6.5)]
        print('MS and RGs selection: from', np.size(simsTestChi), 'selected', np.size(simsTest))  
    else:
        simsTest = simsTestChi
