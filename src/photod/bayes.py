...
def makeBayesEstimates3D(catalog, fitColors, locusData, locus3DList, ArGridList, priorsRootName, outfile, \
    ...
    ArCoeff, ArGridMediumMax, ArGridSmallMax, FeH1d, Mr1d, dFeH, dMr, iEnd, iStart, mdLocus, priorGrid, priorind, reddCoeffs, xLabel, yLabel = _setup_bayes3d(
        ArGridList, MrColumn, catalog, iEnd, iStart, locusData, priorsRootName)

    #######################################################################
    loop_bayes_3d(ArCoeff, ArGridList, ArGridMediumMax, ArGridSmallMax, FeH1d, Mr1d, catalog, dFeH, dMr,
                  fitColors, iEnd, iStart, locus3DList, mdLocus, myStars, priorGrid, priorind, reddCoeffs,
                  verbose, xLabel, yLabel)

    # store results
    # writeBayesEstimates(catalog, outfile, iStart, iEnd, do3D=True)


def loop_bayes_3d(ArCoeff, ArGridList, ArGridMediumMax, ArGridSmallMax, FeH1d, Mr1d, catalog, dFeH, dMr,
                  fitColors, iEnd, iStart, locus3DList, mdLocus, myStars, priorGrid, priorind, reddCoeffs,
                  verbose, xLabel, yLabel):
    ## loop over all stars (can be trivially parallelized)
    for i in range(iStart, iEnd):
        Ar1d, locus3Dok = step_1(ArCoeff, ArGridList, ArGridMediumMax, ArGridSmallMax, catalog, i,
                                 locus3DList, verbose)
        ...
        chi2map = lt.getPhotoDchi2map3D(i, fitColors, reddCoeffs, catalog, locus3Dok, ArCoeff,
                                        masterLocus=True)
        ...
        margpostAr, margpostFeH, margpostMr = postprocess(Ar1d, FeH1d, Mr1d, catalog, dAr, dFeH, dMr, i,
                                                          likeCube, postCube, priorCube)
        ...
        if (i in myStars):
            ...
            show3Flat2Dmaps(priorCube[:, :, indA], likeCube[:, :, indA], postCube[:, :, indA], mdLocus,
                            xLabel, yLabel, logScale=True, x0=FeHStar, y0=MrStar)
            showMargPosteriors3D(Mr1d, margpostMr, 'Mr', 'p(Mr)', FeH1d, margpostFeH, 'FeH', 'p(FeH)', Ar1d,
                                 margpostAr, 'Ar', 'p(Ar)', MrStar, FeHStar, ArStar)
            # these show marginal 2D and 1D distributions (aka "corner plot")
            showCornerPlot3(postCube, Mr1d, FeH1d, Ar1d, mdLocus, xLabel, yLabel, logScale=True, x0=FeHStar,
                            y0=MrStar, z0=ArStar)
            # Qr vs. FeH posterior and marginal 1D distributions for Qr and FeH
            Qr1d, margpostQr = showQrCornerPlot(postCube, Mr1d, FeH1d, Ar1d, x0=FeHStar, y0=MrStar, z0=ArStar,
                                                logScale=True)
            ...
            print('Qr: true=', MrStar + ArStar, 'estimate=', catalog['QrEst'][i], ' +- ',
                  catalog['QrEstUnc'][i])
...
def step_1(ArCoeff, ArGridList, ArGridMediumMax, ArGridSmallMax, catalog, i, locus3DList, verbose):
    if (verbose):
...
def _setup_bayes3d(ArGridList, MrColumn, catalog, iEnd, iStart, locusData, priorsRootName):
    ...
    return ArCoeff, ArGridMediumMax, ArGridSmallMax, FeH1d, Mr1d, dFeH, dMr, iEnd, iStart, mdLocus, priorGrid, priorind, reddCoeffs, xLabel, yLabel
