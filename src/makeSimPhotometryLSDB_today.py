...
def zi_col_names(df):
    # Convert TRILEGAL column names to Željko's notation.
    df = df.rename(columns={'m_h': 'FeH', 'gc': 'GC', 'label': 'pop'})
    return df

def makeSimLSSTcatFromTRILEGAL(cat, MSRGlocus=MSRGlocus, WDlocusH=WDlocusH, WDlocusHe=WDlocusHe, verbose=True):
    # cat = pd.DataFrame(cat)
    # 0) Convert TRILEGAL column names to Željko's notation and insert Mr column.
    ...
    # cat = lt.readAstroLabCatalog(infile, verbose=verbose) LP: this should not be needed as the 
    # function should operate on the chunk passed by LSDB.
    ...
    cat['FeH'] = cat['FeH'].where(cat['GC']==1, cat['FeH']-0.3)
    # shift halo stars to higher [Fe/H] by 0.1 dex
    cat['FeH'] = cat['FeH'].where(cat['GC']==3, cat['FeH']+0.1)
    ...
    # LP commented out if verbose: print('downselected to:', np.size(tGoodMSRG), 'fraction:', np.size(tGoodMSRG)/np.size(catMSRG)) 
    ...
    try:
        ...
        ## Because catWD was empty
        print("Exception")
        tGoodOK = tGoodMSRG
    # LP commented out if verbose: print('Generated model colors, Nsource=', len(tGoodOK))
    ...
    # LP commented out if verbose: print('Final sample:', np.size(tGood4sims), np.size(tGood4sims)/np.size(tGoodOK))
