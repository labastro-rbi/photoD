...
def get2Dmap_ZI(sample, labels, metadata):
    data = np.vstack([sample[labels[0]], sample[labels[1]]])
    kde = gaussian_kde(data)
...
def dumpPriorMaps_testing(sample, fileRootname, pix, show2Dmap=False, verbose=True, NrowMax=200000):
    ...
    foutsum.write(" rMin    rMax   Ntotal     Amin   Amed    Amax     pMS      ppMS     pAGB      pWD        pA1       pA2      pA3      pA4       ptnD     ptkD       pH        pB        pMC \n")
    print("Healpix: ", pix, "\n---------------------------------------")
    for rind, r in enumerate(rGrid):
        ...
        if (len(tS)>NrowMax):
            ...
            if verbose: print('subsampled for 2D map to n=', NrowMax)
        else:
            tSmap = tS
...
def dumpPriorMaps(sample, fileRootname, show2Dmap=True, verbose=True, NrowMax=200000):
    ...
    for rind, r in enumerate(rGrid):
        ...
        # select subsample
        tS = sample[(sample[labels[2]]>rMin)&(sample[labels[2]]<rMax)]
        if verbose: print('r=', rMin, 'to', rMax, 'N=', len(sample), 'Ns=', len(tS), type(tS)) #, tS.index)
        if (len(tS)>NrowMax):
            tSmap = tS.sample(n=NrowMax)
            if verbose: print('subsampled for 2D map to n=', NrowMax)
        else:
...
def makeBayesEstimates3D_profiling(catalog, fitColors, locusData, locus3DList, ArGridList, priorsRootName, outfile, \
    ...
    # Mr and FeH 1-D grid properties extracted from locus data (same for all stars)
    xLabel, yLabel, FeH1d, Mr1d, dFeH, dMr, MrFaint, MrBright = grid(locusData)

    print('Mr1d=', np.min(Mr1d), np.max(Mr1d), len(Mr1d))
    print('MrBright, MrFaint=', MrBright, MrFaint)
    
    # setup arrays for holding results
    initialize_columns(catalog) 
    ...
    for i in range(iStart, iEnd):
        ...
        if (0): ## Always false
            ...
            star_locus_model_3d(catalog, fitColors, locusData, ArCoeff, reddCoeffs, FeH1d, Mr1d, i)
        else:
            ...
            ArMax, ArMin = ar_min_max(catalog, ArCoeff, i)
            ...
            if restrictLocus:  ## Just for testing
                ...
                L, locus3Dok = locus_3D_cuts(ArMax, locus3D, minMr, maxMr, minFeH, maxFeH)
                print('Locus from', np.size(L), 'to', np.size(locus3Dok))

                # also need to reset grids 
                FeH1d, Mr1d, mdLocus = reset_grids(Ar1d, minMr, maxMr, minFeH, maxFeH, locus3Dok)
        ...
        if (restrictLocus):
            ...
            # read maps with priors (and interpolate on the Mr-FeH grid given by locusData, which is same for all stars)
            priorGrid, priorind = restrict_locus(catalog, locusData, priorsRootName, ArMax, minMr, maxMr, minFeH, maxFeH)

        ## generate 3D (Mr, FeH, Ar) prior from 2D (Mr, FeH) prior using uniform prior for Ar    
        priorCube, postCube = prior_3d_from_2d(priorGrid, priorind, FeH1d, Mr1d, i, Ar1d, likeCube)
...
def prior_3d_from_2d(priorGrid, priorind, FeH1d, Mr1d, i, Ar1d, likeCube):
    ...
    return priorCube,postCube

def restrict_locus(catalog, locusData, priorsRootName, ArMax, minMr, maxMr, minFeH, maxFeH):
    L = locusData 
    ...
    priorGrid = readPriors(rootname=priorsRootName, locusData=smallLocus)
            # get prior map indices using observed r band mags
    priorind = getPriorMapIndex(catalog['rmag'])
    return priorGrid,priorind

def reset_grids(Ar1d, minMr, maxMr, minFeH, maxFeH, locus3Dok):
    ...
    return FeH1d,Mr1d,mdLocus

def locus_3D_cuts(ArMax, locus3D, minMr, maxMr, minFeH, maxFeH):
    L = locus3D
    locus3Dok = L[(L['Ar']<=ArMax)&(L['Mr']>minMr)&(L['Mr']<maxMr)&(L['FeH']>minFeH)&(L['FeH']<maxFeH)]
    return L,locus3Dok

def ar_min_max(catalog, ArCoeff, i):
    ArMax = ArCoeff[0]*catalog['Ar'].iloc[i] + ArCoeff[1]
    ArMin = ArCoeff[3]*catalog['Ar'].iloc[i] + ArCoeff[4]
    return ArMax,ArMin

def star_locus_model_3d(catalog, fitColors, locusData, ArCoeff, reddCoeffs, FeH1d, Mr1d, i):
...
def initialize_columns(catalog):
    catalog['MrEst'] = 0.0*catalog['Mr'] - 99 
    catalog['MrEstUnc'] = 0.0*catalog['Mr'] -1 
    catalog['FeHEst'] = 0.0*catalog['Mr'] - 99
    catalog['FeHEstUnc'] = 0.0*catalog['Mr'] - 1 
...
def grid(locusData):
    ...
    return xLabel,yLabel,FeH1d,Mr1d,dFeH,dMr,MrFaint,MrBright
