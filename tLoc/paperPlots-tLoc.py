...
def compare2isochrones(dataDF, isoDF1, isoDF2, alpha1=0.05, alpha2=0.8, title="", plotname='compare2isochrones.png'):
    ...
    def connectDots(ax, x, y, s, c, cmap, alpha, Npts=3):
        for i in range(1, len(x)):
            xGrid = np.linspace(x[i-1], x[i], Npts)
            yGrid = y[i-1] + (y[i]-y[i-1])/(x[i]-x[i-1])*(xGrid-x[i-1])
            cGrid = 0*xGrid + c[i-1]
            ax.scatter(xGrid, yGrid, s=s, c=cGrid, cmap=cmap, alpha=alpha)                                          
            # ax.plot([x[i-1],x[i]], [y[i-1],y[i]], c='blue', cmap=cmap, alpha=alpha)
            # ax.plot([x[i-1],x[i]], [y[i-1],y[i]], c='blue')
    ...
    connectDots(ax[0,0], isoDF1['gi'], isoDF1['Mr'], 0.1, isoDF1['FeH'], plt.cm.jet, alpha1)
    ...
    connectDots(ax[0,1], isoDF2['gi'], isoDF2['Mr'], 0.1, isoDF2['FeH'], plt.cm.jet, alpha2)
...
def analyzeGaiaStripe82(df, ugShift=0.0, piSNRmin=10):
    ...
    # select bright stars
    B = df[(df['u_mMed']<21)]
    print('from', len(df), 'stars, selected', len(B), 'B with u<21')

    # select bright blue stars
    BB = df[(df['gr0']>0.2)&(df['gr0']<0.6)&(df['u_mMed']<21)]
    print('from', len(df), 'stars, selected', len(BB), 'BB with 0.2<g-r<0.6 and u<21')
    # and now select parallax sample
    Pi = BB[(BB['piSNR']>piSNRmin)&(BB['MrPi0']>4)]
    print('from', len(BB), 'BB stars, selected', len(Pi), 'Pi with piSNR>', piSNRmin, 'and MrPi>4')
    ...
    # 2) BB sample
    BB['dMr'] = BB['MrPho0'] - BB['photoMr0'] 
    BB['dMrBayes2'] =  BB['MrEst'] - BB['MrPho0']
    print(' ')
    print('BB sample:', len(BB))
    print('agreement Bayes FeH with photom FeH:', bt.getMedianSigG(bt.basicStats(BB, 'dFeH'))) 
    print('agreement photom Mr with MrPho0:', bt.getMedianSigG(bt.basicStats(BB, 'dMr'))) 
    print('agreement photom Mr with Bayes Mr:', bt.getMedianSigG(bt.basicStats(BB, 'dMrBayes'))) 
    print('agreement MrPho0 with Bayes Mr:', bt.getMedianSigG(bt.basicStats(BB, 'dMrBayes2'))) 

    # 3) B sample
    B['dMr'] = B['MrPho0'] - B['photoMr0'] 
    B['dMrBayes2'] =  B['MrEst'] - B['MrPho0']
    print(' ')
    print('B sample:', len(B))
    print('agreement Bayes FeH with photom FeH:', bt.getMedianSigG(bt.basicStats(B, 'dFeH'))) 
    ...
    print('agreement MrPho0 with Bayes Mr:', bt.getMedianSigG(bt.basicStats(B, 'dMrBayes2'))) 
...
def gaiaStripe82plot1(df, title="", plotname='gaiaStripe82plot1.png'):
    ...
    def plotBinnedMedians(i, j, x, y, xMin, xMax, Nbin):
        ...
        ax[i,j].plot(xBin, medianBin+sigGbin*np.sqrt(nPts)/np.sqrt(np.pi/2), c='red')
        ax[i,j].plot(xBin, medianBin-sigGbin*np.sqrt(nPts)/np.sqrt(np.pi/2), c='red')
        #ax[i,j].plot([xMin, xMax], [0,0], c='yellow')   
        ax[i,j].plot([-1000, 1000], [0,0], c='yellow')   

    symbSize = 0.1
    yLabel = 'MrGeo - photoMr'
    yLabel = 'MrGeo - SDSS Bayes Mr'
    
    ax[0,0].scatter(df['u_mMed'], df['dMr0'], s=symbSize, c=df['MrPi0'], cmap=plt.cm.jet, alpha=alpha)
    ax[0,0].set_xlim(15.5, 19.0)
    ax[0,0].set_ylim(-1.0, 1.0)
    ax[0,0].set_xlabel('u mag')
    ax[0,0].set_ylabel(yLabel)
    plotBinnedMedians(0, 0, df['u_mMed'], df['dMr0'], 16, 19, 12)
    
    ax[0,1].scatter(df['piSNR'], df['dMr0'], s=symbSize, c=df['MrPi0'], cmap=plt.cm.jet, alpha=alpha)
    ...
    ax[1,0].scatter(df['gr0'], df['dMr0'], s=symbSize, c=df['MrPi0'], cmap=plt.cm.jet, alpha=alpha)
    ax[1,0].set_xlim(0.2, 0.62)
    ax[1,0].set_ylim(-1.0, 1.0)
    ax[1,0].set_xlabel('g-r')
    ax[1,0].set_ylabel(yLabel)
    plotBinnedMedians(1, 0, df['gr0'], df['dMr0'], 0.25, 0.6, 7)

    ax[1,1].scatter(df['photoFeH0'], df['dMr0'], s=symbSize, c=df['MrPi0'], cmap=plt.cm.jet, alpha=alpha)
    #ax[1,1].scatter(df['FeHEst'], df['dMr0'], s=symbSize, c=df['MrPi0'], cmap=plt.cm.jet, alpha=alpha)
    ax[1,1].set_xlim(-1.2, 0.1)
    ax[1,1].set_ylim(-1.0, 1.0)
    ax[1,1].set_xlabel('SDSS photometric [Fe/H]')
    #ax[1,1].set_ylabel(yLabel)
    plotBinnedMedians(1, 1, df['photoFeH0'], df['dMr0'], -1.0, 0.0, 8)
...
def gaiaStripe82plot2(df, title="", plotname='gaiaStripe82plot2.png'):
    ...
    ax[1,1].set_xlim(-2.3, 0.0)
    ax[1,1].set_ylim(-1.0, 1.0)
    ax[1,1].set_xlabel('SDSS photometric [Fe/H]')
    ax[1,1].set_ylabel(yLabel)
    plotBinnedMedians(1, 1, df['photoFeH0'], df['dMr'], -2.1, -0.1, 12)
...
def gaiaStripe82plot3(df, title="", plotname='gaiaStripe82plot3.png'):
    ...
    ax[0,0].scatter(df['u_mMed'], df['dMr'], s=symbSize, c='b', cmap=plt.cm.jet, alpha=alpha)
    ax[0,0].set_xlim(15.5, 21.2)
    ax[0,0].set_ylim(-1.0, 1.0)
    ax[0,0].set_xlabel('u mag')
    ax[0,0].set_ylabel(yLabel)
    ...
    ax[0,1].scatter(df['ug0'], df['dMr'], s=symbSize, c='b', cmap=plt.cm.jet, alpha=alpha)
    ax[0,1].set_xlim(0.7, 2.8)
    ax[0,1].set_ylim(-1.0, 1.0)
    ax[0,1].set_xlabel('u-g')
    ax[0,1].set_ylabel(yLabel)
    plotBinnedMedians(0, 1, df['ug0'], df['dMr'], 0.8, 2.6, 12)

    ax[1,0].scatter(df['gi0'], df['dMr'], s=symbSize, c='b', cmap=plt.cm.jet, alpha=alpha)
    ax[1,0].set_xlim(0.25, 2.6)
    ax[1,0].set_ylim(-1.0, 1.0)
    ax[1,0].set_xlabel('g-i')
    ax[1,0].set_ylabel(yLabel)
    plotBinnedMedians(1, 0, df['gi0'], df['dMr'], 0.30, 2.3, 12)

    ax[1,1].scatter(df['photoFeH0'], df['dMr'], s=symbSize, c='b', cmap=plt.cm.jet, alpha=alpha)
    ax[1,1].set_xlim(-2.3, 0.0)
    ax[1,1].set_ylim(-1.0, 1.0)
    ax[1,1].set_xlabel('SDSS photometric [Fe/H]')
    ax[1,1].set_ylabel(yLabel)
...
def gaiaStripe82plot2x(df, title="", plotname='gaiaStripe82plot2.png'):
...
def gaiaStripe82plot4(df, title="", plotname='gaiaStripe82plot4.png'):
